...
> 🌐 **English 지원** — 외국인도 영어로 검색·안내(수령 방법·지도) 받음.
>
...
공식 서비스도 습득물 검색·분실물 신고·유사물품 알림을 한다. **겹치는 건 인정** — FindIt이 이기는 지점은 딱 둘:

1. **검색 품질** — 자연어 + **의미 임베딩**(한↔영 닥스=DAKS·오타·동의어; 실측 BM25 MRR 0.63 → 임베딩 **0.96**) +
   **LLM 판정 근거** + **📷 Vision 사진검색** + **노선 기반 지역 가점**. 관공서 키워드 검색이 가장 약한 "식별자 없는 일반 물건"이 우리 니치.
2. **외국인 접근성** — 관공서는 전부 한국어라 외국인이 못 쓴다. FindIt은 **영어로 검색·판정근거·수령 안내(전화·여권지참 방문·지도·lost112)** 제공.
   *(언어장벽 때문에 관공서가 구조적으로 못 하는 유일한 지점)*

> 포지셔닝: 관공서 **공개데이터 위에 얹은 스마트 검색·매칭 + 외국인 접근성 레이어** (대체 아닌 보완). 실제 수령·권리는 관공서 소관 → HITL 유지.
...
- **웹**: 탭 `[🔍 AI로 찾기 | 📋 전체 둘러보기]` — 등록/사진 → 매칭 결과(근거·썸네일) → HITL 확인 → 내 신고 목록
- **지속 재매칭 + 알림**: 신고 저장(open) → 새 습득물 유입마다 재매칭 → 성립 시 이메일 알림(결과창에서 뒤늦게 이메일 등록도 가능)
